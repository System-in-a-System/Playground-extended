...
class ChatApp extends WindowFrame {
  ...
  connectedCallback () {
    ...
    this.socket.addEventListener('message', e => {
      ...
      // If received messages are not of type 'heartbeat'...
      if (receivedMessage.type !== 'heartbeat') {
        ...
        // Put a channel stamp
        this._controlPanel.textContent = `You are currently listening to ${receivedMessage.channel} channel`
        ...
        const newMessageSound = new window.Audio('./sounds/newmsg.mp3')
  ...
  printMessage (messageToPrint) {
    ...
    // Populate the message 'container' with respective content
    const date = new Date()
