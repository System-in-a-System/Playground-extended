...
// Create a structural template for a custom element 'memory-game'
  ...
  <div id="statistics" 
    style="
      ...
      opacity: 0.6;">
        ...
        <div id="totalTries">🔎: 0</div>
        <div id="totalTimer">⏱: 0</div>
...
`
/**
 * Defines the functionality for 'memory-game' custom element
 * 'memory-game' custom element will serve as a playground for a simple memory game
 *
 * @class MemoryGame
 * @extends {WindowFrame}
 */
class MemoryGame extends WindowFrame {
  constructor () {
    super()

    // Deep clone 'memory-game' template & Append it to the 'content block' from the WindowFrame parent class
    ...
    // Reference the structural parts of the appended 'memory-game' template
  ...
  displayBricks (columns, rows) {
    for (let i = 0; i < columns * rows; i++) {
      ...
      image.addEventListener('click', e => {
        ...
        this._totalTries.textContent = `🔎: ${this._numberOfClicks}`
        ...
        image.setAttribute('src', `images/${this._currentPicture}.jpg`)
        image.style.pointerEvents = 'none'

        // Add sound effects
        const turnSound = new window.Audio('./sounds/turn.mp3')
 ...
 */
  ...
  setTimer () {
    ...
    this._currentTimer = setInterval(() => {
      ...
      this._totalTime++
      this._totalTimer.textContent = `⏱: ${this._totalTime}`
    }, 1000)
  }
  ...
  registerMatch (firstMatch, secondMatch) {
    ...
    // Add sound effects
    const sound = new window.Audio('./sounds/match.mp3')
    sound.volume = 0.1
    sound.play()
  }
  ...
  hidePicture (picture) {
    ...
    if (imagePosition !== -1 && !reservedMatch) {
      ...
      this._board.querySelectorAll('img')[imagePosition].setAttribute('src', 'images/0.jpg')
      this._board.querySelectorAll('img')[imagePosition].style.pointerEvents = 'auto'

      this._board.querySelectorAll('img')[imagePosition2].setAttribute('src', 'images/0.jpg')
   ...
   * Offers an access to the global score list
   *
   * @memberof MemoryGame
   */
  finishTheGame () {
...
// Register 'memory-game' custom element
